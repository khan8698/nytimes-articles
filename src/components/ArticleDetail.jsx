        ...
        <>
          {imageUrl && (
            <div className="mb-4 sm:mb-6">
              <img src={imageUrl} alt={article?.title} className="w-full h-auto rounded-lg object-contain" />
              <p className="text-sm text-gray-600 mt-2">{article?.media?.[0]?.caption}</p>
            </div>
          )}
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-800 mb-4">{article?.title}</h1>
          <p className="text-gray-700 text-base sm:text-lg mb-4">{article?.abstract}</p>
          <div className="text-gray-600 text-xs sm:text-sm mb-6">
            ...
            {article?.geo_facet && article.geo_facet.length > 0 && (
              <p><strong>Geographical Locations:</strong> {article.geo_facet.join(', ')}</p>
            )}
          </div>
          <p>
            <a href={article?.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 font-semibold text-base sm:text-lg hover:underline">
              Read more
            </a>
          </p>
        </>
